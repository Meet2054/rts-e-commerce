      ...
      <div ref={modalRef} className="bg-white rounded-xl shadow-xl max-w-lg w-full mx-4 p-8 relative">
        ...
        <div className="flex gap-4 mb-6">
          {[1, 2, 3].map((i) => (
            <div key={i} className="w-32 h-24 border rounded-lg flex items-center justify-center relative bg-[#F1F2F4]">
              <span className="absolute top-2 right-2 text-gray-400 text-lg cursor-pointer">×</span>
        ...
        <form className="flex flex-col gap-4">
          <div>
            ...
            <input type="text" className="w-full border rounded px-4 py-2 bg-[#F1F2F4]" defaultValue="HP Toner Cartridge" />
          </div>
          <div>
            <label className="block font-semibold mb-1 text-black">Description:</label>
            <textarea className="w-full border rounded px-4 py-2 bg-[#F1F2F4]" rows={2} defaultValue="High-quality toner cartridge compatible with HP LaserJet printers. Long-lasting and reliable print performance." />
          </div>
          <div>
            <label className="block font-semibold mb-1 text-black">Category:</label>
            <input type="text" className="w-full border rounded px-4 py-2 bg-[#F1F2F4]" defaultValue="Ink & Toner" />
          </div>
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="block font-semibold mb-1 text-black">Price:</label>
              <input type="text" className="w-full border rounded px-4 py-2 bg-[#F1F2F4]" defaultValue="$ 3500" />
            </div>
            <div className="flex-1">
              <label className="block font-semibold mb-1 text-black">Stock:</label>
              <input type="text" className="w-full border rounded px-4 py-2 bg-[#F1F2F4]" defaultValue="12" />
            </div>
          </div>
          <button
            type="submit"
